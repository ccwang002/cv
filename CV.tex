% !TEX TS-program = xelatex
% !TEX encoding = UTF-8

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% SIMPLE-RESUME-CV
%% <https://github.com/zachscrivena/simple-resume-cv>
%% This is free and unencumbered software released into the
%% public domain; see <http://unlicense.org> for details.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% INSTRUCTIONS FOR COMPILING THIS DOCUMENT ("CV.tex")
%% TEX ---(XeLaTeX)---> PDF:
%%
%% Method 1: Use latexmk for fully automated document generation:
%%   latexmk -xelatex "CV.tex"
%%   (add the -pvc switch to automatically recompile on changes)
%%
%% Method 2: Use XeLaTeX directly:
%%   xelatex "CV.tex"
%%   (run multiple times to resolve cross-references if needed)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[a4paper,12pt,oneside]{article}
%\documentclass[letterpaper,10pt,oneside]{article}

% Do not stop on errors during compilation.
%\nonstopmode

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PREAMBLE.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\input{CV-Preamble.tex}

% CV Info (to be customized).
\newcommand{\CVTitle}{Liang-Bo Wang's CV}
\newcommand{\CVNote}{Last updated: {\today}}

% PDF settings and properties.
\hypersetup{
pdftitle={\CVTitle},
pdfauthor={Liang Bo Wang},
pdfsubject={http://liang2.tw},
pdfcreator={XeLaTeX},
pdfproducer={},
pdfkeywords={},
pdfpagemode={},
bookmarks=true,
unicode=true,
bookmarksopen=true,
pdfstartview=FitH,
pdfpagelayout=OneColumn,
pdfpagemode=UseOutlines,
hidelinks,
breaklinks}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ACTUAL DOCUMENT.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{document}

%%%%%%%%%%%%%%%
% TITLE BLOCK %
%%%%%%%%%%%%%%%

\title{Liang-Bo Wang}

\begin{subtitle}
%\href{https://www.google.com/maps/place/17+Prime+Avenue,+Springfield,+Pennsylvania+10111,+USA}
%{17 Prime Avenue, Apt 37, Springfield, Pennsylvania 10111, USA}
%\par
\href{mailto:r02945054@ntu.edu.tw}
{r02945054@ntu.edu.tw}
\,\BulletSymbol\,
+886-953-632-348
\,\BulletSymbol\,
\href{http://liang2.tw}
{http://liang2.tw}
\end{subtitle}

\begin{body}

%%%%%%%%%%%%%%%
%% EDUCATION %%
%%%%%%%%%%%%%%%

\section
{Education}
{Education}
{PDF:Education}

\href{http://www.ntu.edu.tw/english/index.html}
{\textbf{National Taiwan University}},
Taipei, Taiwan
\par
\textbf{M.S.} in
\href{http://www.bebi.ntu.edu.tw/English/index.html}
{Biomedical Electronics and Bioinformatics}
\hfill
\DatestampYM{2014}{01} --
\DatestampYM{2016}{06} (expected)
\begin{detail}
    \begin{detailitem}
    \item GPA: 3.85 / 4.0
    \end{detailitem}
\end{detail}


\EntryGapNoBreak
\textbf{B.S.} in
\href{http://www.ee.ntu.edu.tw/en/}
{Electrical Engineering}
\hfill
\DatestampYM{2009}{09} --
\DatestampYM{2014}{01}
\begin{detail}
    \begin{detailitem}
    \item Minor in Economics
    \item GPA: 3.79 / 4.0
    \end{detailitem}
\end{detail}

%%%%%%%%%%%%%%%%%%%%%%%%%
%% RESEARCH EXPERIENCE %%
%%%%%%%%%%%%%%%%%%%%%%%%%

\section
{Research Experience}
{Research Experience}
{PDF:ResearchExperience}

\textbf{Microsoft Research Asia},
Beijing, China
\hfill
\DatestampYM{2014}{07} --
\DatestampYM{2015}{02}
\par
Internship
\begin{detail}
    \begin{detailitem}
    \item Advisor: Eric Y. Chang
    \item Topic: Cancer Image and Pathway Analysis
    \item Applied convolutional neural network to histopathology image analysis
    \end{detailitem}
\end{detail}


\BigEntryGapNoBreak
\textbf{Biomedical Engineering and Bioinformatics},
National Taiwan University
\par
Graduate Research
\hfill
\DatestampYM{2014}{01} --
Present
\begin{detailitem}
    \item Advisor: Eric Y. Chuang
    \item Topic: Integrative analysis of copy number variation, gene expression, and miRNA regulation on cancer; drug treatment prediction on lung cancer cell line
\end{detailitem}


\EntryGapNoBreak
Undergraduate Research% (Advisor: Dr. Eric. Y. Chuang)
\hfill
\DatestampYM{2012}{09} --
\DatestampYM{2014}{01}
\begin{detail}
    \begin{detailitem}
    \item Advisor: Eric Y. Chuang
    \item Topic: DNA-seq, RNA-seq and small RNA-seq data processing
    \item Project: \href{https://github.com/BioCloud-TW/nextbiopy}{Nextbiopy}, a web-based NGS pipeline report generator
    \end{detailitem}
\end{detail}

\EntryGapNoBreak
Undergraduate Research% (Advisor: Dr. Wei-Cheng Tian)
\hfill
\DatestampYM{2011}{09} --
\DatestampYM{2012}{11}
\begin{detail}
    \begin{detailitem}
    \item Advisor: Wei-Cheng Tian
    \item Project: CMOS-based tactile sensor system for blood pressure measurement
    \end{detailitem}
\end{detail}

%%%%%%%%%%%%%%%%%%
%% PUBLICATIONS %%
%%%%%%%%%%%%%%%%%%

\section
{Publications}
{Publications}
{PDF:Publications}

\subsection
{Journals}
{Journals}
{PDF:Journals}

\begin{publication}[series=pub, widest*=100]

    \item Y.-P. Lai$^*$, \textbf{L.-B. Wang}$^*$, L.-C. Lai, M.-H. Tsai, T.-P. Lu, and E. Y. Chuang,
        ``iGC–an integrated analysis package of Gene expression and Copy number alteration'',
        Under review.

    \item
        Y. Xu, F. Zhang, Z. Jia, \textbf{L.-B. Wang}, Y. Ai, M. Lai, E. Chang,
        ``Large Scale Tissue Histopathology Image Classification, Segmentation and Visualization via Deep Convolutional Activation Features'',
        Under review.

    \item
        \textbf{L.-B. Wang}, E. Y. Chuang, T.-P. Lu,
        ``Identification of predictive biomarkers for ZD-6474 in lung cancer'',
        \textit{Translational Cancer Research}, vol. 4, no. 4,
        \DatestampYM{2015}{08}.

    \item
        C.-Y. Lee$^*$, Y.-C. Chiu$^*$, \textbf{L.-B. Wang}$^*$, Y.-L. Kuo$^*$, E. Y. Chuang, L.-C. Lai, M.-H. Tsai,
        ``Common applications of next-generation sequencing technologies in genomic research'',
        \textit{Translational Cancer Research},
        \DatestampYM{2013}{02}.
\end{publication}

\BigEntryGap
\subsection
{Conference Papers}
{Conferences Papers}
{PDF:Conference Papers}

\begin{publication}[widest*=100]  %resume=pub to resume the publication counter

    \item
        \textbf{L.-B. Wang}, S.-Y. Dai, T.-P. Lu, L.-C. Lai, M.-H. Tsai, E. Y. Chuang,
        ``Nextbiopy: Utilities for next-generation sequencing data processing'',
        \textit{9th Asian Pacific Conference on Medical and Biological Engineering (APCMBE)},
        Tainan, Taiwan,
        \DatestampYM{2014}{10}.

    \item
        C.-Y. Lee$^*$, \textbf{L.-B. Wang}$^*$, M.-H. Tsai, L.-C. Lai, E. Y. Chuang,
        ``Identification of novel miRNAs in breast data of the next generation sequencing using miRDeep2 and Galaxy'',
        \textit{AACR 104th Annual Meeting},
        Washington, DC, U.S.A.,
        \DatestampYM{2013}{04}.

    \item
        Y.-C. Lin, C.-J. Hsieh, \textbf{L.-B. Wang} (presenter), J.-C. Liou, W.-C. Tian,
        ``CMOS MEMS metal-based tactile sensors development'',
        \textit{AVS 59th International Symposium \& Exhibition},
        Florida, U.S.A.,
        \DatestampYM{2012}{10}.

    \item
        C.-T. Sun, Y.-C. Lin, C.-J. Hsieh, J.-C. Liou, \textbf{L.-B. Wang,} W.-C. Tian,
        ``A linear-response CMOS-MEMS capacitive tactile sensor'',
        \textit{IEEE Sensors 2012},
        Taipei, Taiwan,
        \DatestampYM{2012}{10}.

\end{publication}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% ACADEMIC HONORS & AWARDS %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section
{Honors\newline
\& Awards}
{Honors \& Awards}
{PDF:Honors Awards}

\textbf{Outstanding Summer Research Award},
\hfill
\DatestampY{2012}
\par
Center of Genomic Medicine, National Taiwan University
\begin{detail}
    \begin{detailitem}
    \item Research Topic: miRNA discovery using NGS data
    \end{detailitem}
\end{detail}

\EntryGapNoBreak
\textbf{Gold medal},
International Genetically Engineered Machine (iGEM)
\hfill
\DatestampY{2012}
\begin{detail}
    \begin{detailitem}
    \item Topic: A demonstration of PepdEx with mind-altering bacteria
    \item Role: Biological circuit (gene network) simulation and wiki page construction
    \end{detailitem}
\end{detail}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% PROFESSIONAL AFFILIATIONS & ACTIVITIES %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% \section
% {Professional Affiliations\newline
% \& Activities}
% {Professional Affiliations \& Activities}
% {PDF:ProfessionalAffiliationsActivities}
%
% \href{http://www.example.com/my-society}
% {\textbf{Society of Professional Earth Scientists}},
% New York, USA
% \par
% Member
% \hfill
% \DatestampY{2009} --
% Present

%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% OTHER WORK EXPERIENCE %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section
{Work Experience}
{Work Experience}
{PDF:Work Experience}

\href{http://pinkoi.com}
{\textbf{Pinkoi}},
Taipei, Taiwan
\hfill
\DatestampYM{2015}{06} --
\DatestampYM{2015}{09}
\par
Internship
\begin{detail}
    \begin{detailitem}
    \item An online marketplace for unique and original design goods in Asia
    \item Topic: Product search improvement
    \item Improved Chinese text segmentation on ElasticSearch and added
        preliminary semantic search built on Word2Vec
    \end{detailitem}
\end{detail}


%%%%%%%%%%%%
%% SKILLS %%
%%%%%%%%%%%%

\section
{Programming Skills}
{Programming Skills}
{PDF:Programming Skills}

\subsection
{Languages}
{Languages}
{PDF:Languages}

Python (both scientific and web stack),
R,
\LaTeX,
C, SQL, Web frontend

% \subsection
% {Tools}
% {Tools}
% {PDF:Tools}
%
% \begin{detail}
%     \begin{detailitem}
%     \item Bioinfo: samtools, seqtk, FastQC, Tophat
%     \item Python: Numpy, Scipy, Pandas, sckit-learn, matplotlib, seaborn, gensim, Django, Flask, Bottle
%     \item R: rpy2, ggplot2, (d)plyr, data.tables
%     \item Database: PostgreSQL, SQLite
%     \item OS: Debian Linux, OSX
%     \end{detailitem}
% \end{detail}

%%%%%%%%%%%%%%%%%%%%%%%
%% CAMPUS ACTIVITIES %%
%%%%%%%%%%%%%%%%%%%%%%%

\section
{Extra-\newline
curricular\newline
Activities}
{Extracurricular Activities}
{PDF:Extracurricular Activities}

\href{http://pycon.tw}{\textbf{PyCon Taiwan}} Organizer, Taipei, Taiwan
\hfill
\DatestampYM{2015}{07} --
Present
\par
Program Chair

\EntryGapNoBreak
\href{http://pycon.tw}{\textbf{PyCon APAC}} Organizer, Taipei, Taiwan
\hfill
\DatestampY{2014},
\DatestampY{2015}
\par
Public Relations Manager

\BigEntryGapNoBreak
\textbf{Open Source Communities}, Taipei, Taiwan
\hfill
\DatestampY{2013} --
Present
\par
Regular Speaker
\begin{detail}
    \begin{detailitem}
    \item Including Taiwan PyCon, Taipei.py, Taiwan R User Group, and PyCon China
    \item Gave over \href{http://blog.liang2.tw/talks/#talks}{10 talks}, including convolutional neural network, text search,\newline
        HDF5, Parallel Computing, and R ggplot2 plotting.
    \end{detailitem}
\end{detail}

\BigEntryGap
\textbf{Photography Club},
National Taiwan University
\hfill
\DatestampY{2009} --
\DatestampY{2013}

President
\hfill
\DatestampYM{2011}{03} --
\DatestampYM{2011}{09}

Leader of Education Division
\hfill
\DatestampYM{2010}{03} --
\DatestampYM{2011}{03}

% \BigEntryGap
% \textbf{Summer Camp for High School},
% \hfill
% \DatestampY{2011},
% \DatestampY{2012}
% \par
% Electrical Engineering, National Taiwan University
%
% Leader of the Photography Division
% \begin{detail}
%     \begin{detailitem}
%     \item Recorded, wrote and produced television news and newspaper per day about camp activities
%     \end{detailitem}
% \end{detail}
\end{body}

%%%%%%%%%%%
% CV NOTE %
%%%%%%%%%%%

\begin{flushright}
\scriptsize%
[\textit{\CVNote}]%
\hspace{2.0mm}\null
\end{flushright}

\label{LastPage}
\end{document}
